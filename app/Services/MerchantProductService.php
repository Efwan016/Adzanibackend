...
class MerchantProductService
{
    ...
    public function assignProductToMerchant(array $data)
    {
        return DB::transaction(function () use ($data) {
            ...
            if (!$warehouseProduct || $warehouseProduct->stock < $data['stock']) {
                throw ValidationException::withMessages(['message' => 'Insufficient stock in warehouse']);
            }
    ...
    public function updateStock(int $merchantId, int $productId, int $newStock, ?int $warehouseId)
    {
        return DB::transaction(function () use($merchantId, $productId, $newStock, $warehouseId)
        {
            ...
            if (!$existing) {
                throw ValidationException::withMessages(['product' => 'Product not assigned to this merchant']);
            }

            if (!$warehouseId) {
                throw ValidationException::withMessages(['warehouse_id' => 'Warehouse ID is required when increasing stock']);
            }
        ...
        if ($newStock > $currentStock) {
            ...
            if (!$warehouseProduct || $warehouseProduct->stock < $diff) {
                throw ValidationException::withMessages(['stock' => ['Insufficient stock in warehouse']
            ]);
            }
    ...
    public function removeProductFromMerchant(int $merchantId, int $productId)
    {
       // $merchant = Merchant::findOrFail($merchantId);
        ...
        if (!$merchant) {
            throw ValidationException::withMessages(['product' => ['merchant not Found.']
        ]);
        }
